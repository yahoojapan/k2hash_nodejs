...
if [ "X${DEBUG_MODE}" = "XINFO" ]; then
	echo "***** RUN *****"
	echo "NODE_PATH=${K2HASH_NODE_PATH} K2HDBGMODE=${DEBUG_MODE} K2HDBGFILE=${DEBUG_LOG} ${SRCTOP}/node_modules/.bin/mocha ${MYSCRIPTDIR}/${CMD_PREFIX}${COMMAND}${CMD_SUFFIX}"
	echo "***************"
fi
NODE_PATH=${K2HASH_NODE_PATH} K2HDBGMODE=${DEBUG_MODE} K2HDBGFILE=${DEBUG_LOG} ${SRCTOP}/node_modules/.bin/mocha ${MYSCRIPTDIR}/${CMD_PREFIX}${COMMAND}${CMD_SUFFIX}
